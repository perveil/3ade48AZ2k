...
create table `advert` (
  ...
  `pos_id` int not null COMMENT '广告位id',
  ...
  `status` int NOT NULL DEFAULT 0 COMMENT '状态 0：开启 1：关闭 2：已结束',
  `create_time` TIMESTAMP NOT NULL DEFAULT current_timestamp COMMENT '创建时间',
	`update_time` TIMESTAMP NOT NULL DEFAULT current_timestamp ON UPDATE current_timestamp COMMENT '更新时间'
)
ENGINE=InnoDB
...
create table `charge` (
  ...
  `charge_type` int NOT NULL COMMENT '充值类型 0：充值 1：礼品卡充值',
  `valid_type` int DEFAULT 0 COMMENT '有效期 0：永不过期 1：有期限',
  `valid_time` varchar(64) default '' COMMENT '有效期限',
  `is_valid` int DEFAULT 0 COMMENT '0：正常 1：关闭 2:已结束',
  `create_time` TIMESTAMP NOT NULL DEFAULT current_timestamp COMMENT '创建时间',
	`update_time` TIMESTAMP NOT NULL DEFAULT current_timestamp ON UPDATE current_timestamp COMMENT '更新时间'
)
ENGINE=InnoDB
